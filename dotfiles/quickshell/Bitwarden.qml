...
PanelWindow {
  ...
  function checkSession() {
    setLoading(true);
    runBwScript("check", "BW_SESSION=\"$1\" bw unlock --check >/dev/null 2>&1; if [ $? -eq 0 ]; then printf 'OK\\n'; else printf 'ERR\\n'; fi", sessionToken);
  }
  ...
  function loadItems() {
    setLoading(true);
    runBwScript("list", "out=$(BW_SESSION=\"$1\" bw list items 2>/dev/null); rc=$?; if [ $rc -eq 0 ]; then printf 'OK\\n%s' \"$out\"; else err=$(BW_SESSION=\"$1\" bw list items 2>&1 >/dev/null); printf 'ERR\\n%s' \"$err\"; fi", sessionToken);
  }

  function loadTotp(itemId) {
    setLoading(true);
    runBwScript("totp", "out=$(BW_SESSION=\"$1\" bw get totp \"$2\" 2>&1); rc=$?; if [ $rc -eq 0 ]; then printf 'OK\\n%s\\n' \"$out\"; else printf 'ERR\\n%s\\n' \"$out\"; fi", sessionToken, itemId);
  }

  function needsReauth(text) {
    var s = (text || "").toLowerCase();
    return s.indexOf("vault is locked") !== -1 || s.indexOf("invalid session") !== -1 || s.indexOf("session key") !== -1 || s.indexOf("not logged in") !== -1 || s.indexOf("log in") !== -1;
  }
  ...
  function unlock() {
    ...
    runBwScript("unlock", "BW_WOFI_MASTER_PASSWORD=\"$1\"; out=$(BW_WOFI_MASTER_PASSWORD=\"$1\" bw unlock --raw --passwordenv BW_WOFI_MASTER_PASSWORD 2>/dev/null); rc=$?; if [ $rc -eq 0 ] && [ -n \"$out\" ]; then printf 'OK\\n%s\\n' \"$out\"; else printf 'ERR\\n'; fi", password);
