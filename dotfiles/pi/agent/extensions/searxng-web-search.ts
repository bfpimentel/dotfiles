...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      limit: Type.Optional(Type.Number({ description: "Maximum number of results to return (default 8, max 20)" })),
      language: Type.Optional(Type.String({ description: "SearXNG language code, e.g. 'en', 'all'" })),
      categories: Type.Optional(Type.String({ description: "SearXNG categories, e.g. 'general', 'it', 'science'" })),
      time_range: Type.Optional(Type.String({ description: "Optional time range: day, week, month, year" })),
    }),
    async execute(_toolCallId, params, signal) {
      ...
      if (!response.ok) {
        ...
        return {
          isError: true,
          content: [{ type: "text", text: `SearXNG search failed: HTTP ${response.status}${body ? `\n${body.slice(0, 1000)}` : ""}` }],
